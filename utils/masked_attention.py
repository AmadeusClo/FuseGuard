...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        if self.mask_flag:
            # 将 attn_mask 的维度从 (B, L, L) 扩展到 (B, 1, L, L)
...
class Mahalanobis_mask(nn.Module):
    ...
    def calculate_prob_distance(self, X):
        XF = torch.abs(torch.fft.rfft(X, dim=-1))    # X:(B, F, seq_len)  XF:(B, F, seq_len//2+1,)
        X1 = XF.unsqueeze(2)                         # (B, F, 1, seq_len//2 + 1)
        X2 = XF.unsqueeze(1)                         # (B, 1, F, seq_len//2 + 1)


        diff = X1 - X2                               # (B, F, F, seq_len//2 + 1
